...
## Driver board + solenoid wiring architecture (July 2026 session)

### Driver design (what exists)
...
- All 84 solenoid leads are identical black → **position is identity**. Sharpie the key name beside each landing hole; log `cell · OUT · key` **at solder time**, one line per wire; type the table into firmware `MAP` when done. Walk mode (fire 0–83, watch which key clicks) is the final verification pass, not the primary method.

### Harness: plate-side +12V distribution (DECIDED)

- The board's +12V rail **stays** (ULN COMs + reservoir cap live on it). Added: **+12V distribution on the plate** — bare ~16 AWG bus wire along each mounting wall, all walls tied by a trunk at one end, fed by **ONE 18 AWG +12V wire from the board rail**. **No ground wire to the plate** — return current comes back through the 84 low-side wires.
- Per solenoid: **high side → ~5 cm hop to its wall bus; low side → 22 AWG stranded grey, cut to length, → its ULN OUT landing hole.** Plate↔board harness = 84 grey + 1 orange = **85 wires (vs 168)**.
- Peak bus load ≈ 7 keys × 300 mA ≈ 2 A — 16 AWG has big margin. If ringing/resets appear under heavy typing, optional 470 µF across the plate bus.
- Solid→stranded transition happens **at the landing hole** (solid on board, stranded off-board), never mid-run.

### Bench triage for the wire mess (photo on file: plate with ~half the solenoids mounted, leads frayed + tangled)

1. Snip or tape every frayed bare tip immediately — shed strands are a 12 V short waiting to happen.
2. Velcro the leads into per-row bundles; work one wall at a time.
3. **Golden rule: cut each wire to length only at the moment it's landed. Never pre-cut.** Length = label.
4. Optional: snap-on printed combs over the wall tops for the low-side runs (plate already printed, so integrated troughs are no longer possible).

### Wave 2 mouse — brainstorm only (NO decision yet)

- Daniel is open to replacing the XY-gantry + B100 plan; wants viral mix of "looks alive / real feats / absurd".
- Shortlist: (1) **animatronic hand riding the mouse** over the hidden gantry — best ROI; (2) **trackball + 2 stepper friction wheels** — simplest, ~$60–80; (3) **5-bar pantograph arm** — ~$60–90; (4) **holonomic mouse rover** — highest novelty/risk. $0 content gag: macOS Mouse Keys ("the robot refuses to touch the mouse") before mouse hardware exists.
